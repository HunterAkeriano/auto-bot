...
async function generatePersonalTarotWeekly() {
    const prompt = `Вибери ТРИ випадкові карти Таро (з повної колоди, 78 карт) для індивідуального передбачення на *тиждень*. Назви ці карти. Склади **реалістичний, жорсткий, об'єктивний** прогноз, де перша карта описує початок тижня, друга — середину, третя — кінець. Прогноз повинен об'єктивно відображати значення карт, включаючи можливі виклики та можливості відповідно до карт. **Особливо, якщо випала негативна карта, її опис має бути чесним та суворим.** Довжина тексту не більше 150 слів. Форматуй назви карт як *[Назва Карти]*.`;
...
async function generatePersonalTarotMonthly() {
    const prompt = `Вибери ОДНУ ключову карту Таро (з повної колоди, 78 карт) для індивідуального передбачення на *місяць*. Назви цю карту. Склади глибокий, змістовний, **реалістичний** прогноз на місяць, описуючи основний енергетичний фокус, можливі виклики, ключові події та об'єктивну головну пораду. **Якщо карта негативна або попереджувальна, детально та суворо опиши ці аспекти.** Довжина тексту не більше 200 слів. Форматуй назву карти як *[Назва Карти]*.`;
...
async function generatePersonalTarotReading() {
    const prompt = `Вибери одну випадкову карту з повної колоди Таро (78 карт) для індивідуального передбачення на день. Надай її назву українською та склади реалістичний, особистісний прогноз. Зверни увагу на ключові аспекти: настрій, енергія, порада. **Прогноз повинен об'єктивно відображати значення карти, включаючи можливі труднощі. Якщо карта має негативний або попереджувальний аспект, опис має бути чесним, суворим і реалістичним.** Формат: *[Назва Карти]*. Далі детальний, особистий прогноз. Довжина тексту не більше 100 слів.`;
...
async function generateHoroscope(sign, promptStyle, dayContext) {
    ...
    if (promptStyle === 'serious') {
        basePrompt = `Склади інформативний, нейтральний, **об'єктивний** прогноз на ${dayContext} для знаку зодіаку ${sign} українською мовою. **Прогноз повинен відображати як позитивні, так і можливі складні аспекти.** Не використовуй надмірну кількість емодзі, окличних знаків чи сленгу. Дотримуйся ділового або психологічного тону. Довжина тексту прогнозу НЕ ПОВИННА перевищувати ${wordLimit} слів.`;
...
async function generateTarotReading(dayContext) {
    ...
    const prompt = `Вибери одну випадкову карту з повної колоди Таро (78 карт, включаючи Молодші Аркани). Надай її назву українською та **об'єктивний** опис її значення для прогнозу на ${dayContext}. Опис повинен відображати **повний спектр значення карти, включаючи попередження або виклики, якщо вони є. Якщо карта негативна, опис має бути суворим та реалістичним.** Формат: *[Назва Карти]*. Опис та прогноз. Довжина тексту не більше 70 слів.${exclusion}`;
...
async function generateCompatibilityReading(sign1, sign2) {
    const prompt = `Склади детальний, **реалістичний** опис сумісності знаків зодіаку *${sign1}* та *${sign2}* у сфері стосунків. Виділи **сильні сторони та потенційні зони конфлікту чи труднощів** цієї пари, та дай **збалансовану** пораду. Загальна довжина тексту НЕ ПОВИННА перевищувати 150 слів.`;
    return generateContent(prompt, `Compatibility: ${sign1} & ${sign2}`);
}

async function generateWeeklyHoroscopeReading(sign) {
    const prompt = `Склади інформативний, **об'єктивний** прогноз для знаку зодіаку *${sign}* на поточний тиждень. Опиши основні тенденції (робота, фінанси, особисте життя) одним-двома лаконічними реченнями. Прогноз має включати **як успіхи, так і можливі труднощі.** Довжина тексту НЕ ПОВИННА перевищувати 35 слів.`;
    return generateContent(prompt, sign);
}

async function generateNumerologyReading(number, dateString) {
    const prompt = `Склади **реалістичний** прогноз українською мовою для *Числа Дня ${number}* на дату ${dateString}. Опиши ключові тенденції цього числа та дай пораду, як використати його енергію. **Опис повинен включати як сприятливі, так і потенційно складні аспекти дня.** Довжина тексту НЕ ПОВИННА перевищувати 80 слів.`;
...
async function generateDailyTarotAnalysis(dayContext) {
    ...
    const prompt = `Вибери ОДНУ випадкову карту з повної колоди Таро (78 карт, включаючи Молодші Аркани). Надай її назву українською та склади на її основі **об'єктивний, детальний, чесний** "розбір таро" на ${dayContext}. Опиши ключове значення, дай психологічну пораду та поясни, як її енергія впливає на вечір. **Аналіз має чесно відображати значення карти. Якщо карта має негативний або попереджувальний аспект, опис має бути суворим, реалістичним і без прикрас.** Формат: *[Назва Карти]*. Потім детальний аналіз.${exclusion} Довжина тексту не більше 120 слів.`;
...
async function publishTarotReading() {
    ...
    message += formatTarotCardBold(tarotText) + '\n\n';
...
async function publishDailyTarotAnalysis() {
    ...
    message += formatTarotCardBold(analysisText) + '\n\n';
